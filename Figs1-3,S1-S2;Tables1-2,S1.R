...
# Figure S1
png("ecography_10nov23/rev1/FigureS1_all.png", width = 12, height = 6)
ggplot() +
    geom_sf(data = worldmap, color = "gray", fill = "lightgray") +
    xlim(-180, 0) + 
    geom_sf(data = gbif_data_clean2_sf, aes(colour = species)) + 
    theme_void() +
  theme(legend.position="none")
dev.off()

pdf("ecography_10nov23/rev1/FigureS1_sep.pdf", width = 12, height = 6)
...
# Figure S2
pdf("ecography_10nov23/rev1/FigureS2.pdf")
plot(log(area_alp) ~ log(area_sdm), pch = 16, 
     ylab = expression(paste("Range size (km"^"2", "): Alpha hull polygon")),
     xlab = expression(paste("Range size (km"^"2", "): Species distribution modeling")))
reg_rs <- lm(log(area_alp) ~ log(area_sdm))
abline(reg_rs)
dev.off()
...
col_points <- rgb(190/255, 190/255, 190/255, alpha = 0.7)
col_line <- turbo(20)[20]

p1 <- ggplot(fulldt_analyses) + 
  geom_point(aes(x = speciesDBR, y = scale_area), colour = col_points) +
  geom_abline(aes(intercept = summary(res_pgls_spp)$coefficients[1, 1], 
                  slope = summary(res_pgls_spp)$coefficients[2, 1]), 
              linewidth = 1, linetype = 2, colour = col_line) + 
  labs(x = "Diet breadth", y = expression(paste("Range size (km"^"2", ")"))) +
  theme_light() + 
...
p2 <- ggplot(fulldt_analyses) + 
  geom_point(aes(x = scale_pc1, y = scale_area), colour = col_points) +
  geom_abline(aes(intercept = summary(res_pgls_spp)$coefficients[1, 1], 
                  slope = summary(res_pgls_spp)$coefficients[3, 1]), colour = col_line, 
              linewidth = 1) + 
  labs(x = "PC1", y = "") + 
  theme_light() + 
  theme(panel.grid.major = element_blank(), 
...
p3 <- ggplot(fulldt_analyses) + 
  geom_point(aes(x = scale_pc2, y = scale_area), colour = col_points) +
  geom_abline(aes(intercept = summary(res_pgls_spp)$coefficients[1, 1], 
                  slope = summary(res_pgls_spp)$coefficients[4, 1]), colour = col_line, 
...
p4 <- ggplot(fulldt_analyses) + 
  geom_point(aes(x = scale_pc3, y = scale_area), colour = col_points) +
  geom_abline(aes(intercept = summary(res_pgls_spp)$coefficients[1, 1], 
                  slope = summary(res_pgls_spp)$coefficients[4, 1]), colour = col_line, 
              linewidth = 1, linetype = 2) + 
  labs(x = "PC3", y = "") + 
...
p5 <- ggplot(fulldt_analyses) + 
  geom_point(aes(x = scale_nbtemp, y = scale_area), colour = col_points) +
  geom_abline(aes(intercept = summary(res_pgls_spp)$coefficients[1, 1], 
                  slope = summary(res_pgls_spp)$coefficients[7, 1]), colour = col_line, 
              linewidth = 1) + 
  labs(x = "Temperature niche breadth", y = "") + 
  theme_light() + 
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        legend.position="none") +
...
p6 <- ggplot(fulldt_analyses) + 
  geom_point(aes(x = scale_nbprec, y = scale_area), colour = col_points) +
  geom_abline(aes(intercept = summary(res_pgls_spp)$coefficients[1, 1], 
                  slope = summary(res_pgls_spp)$coefficients[6, 1]), colour = col_line, 
              linewidth = 1) + 
  labs(x = "Precipitation niche breadth", y = "") + 
  theme_light() + 
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        legend.position="none") +
...
p1 <- ggplot(fulldt_analyses_plot) + 
  ...
  geom_abline(data = abline, 
              ...
              linewidth = 1, linetype = 2) + 
  scale_colour_manual(values = c(cols[1], cols[3], cols[4], cols[2])) +
...
pdf("ecography_10nov23/rev1/FigureS4.pdf", width = 11)
