...
dietbreadthdt_clean <- read.csv("data/dietbreadthdt_all_clean.csv", row.names = 1)
...
for (i in 1:nrow(gbif_taxon_keys)) {
  ...
  doi[i] <- gbif_citation(occ_download_get(gbif_data))$download[1]
...
#save(gbif_downloads, file = "data/gbif_downloads_12jun24.RData")

write.csv(data.frame(spp = gbif_taxon_keys[, 1], 
                     taxonKey = gbif_taxon_keys[, 2],
                     doi = do.call(c, doi)), "tables/doi_gbif.csv")
