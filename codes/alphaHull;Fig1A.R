...
dietbreadth <- read.csv("data/dietbreadthdt_clean.csv", row.names = 1)
...
master <- read.csv("data/MasterTable11.csv", strip.white = TRUE,
                   na.strings = c("", "NA"), stringsAsFactors = TRUE)
habitat <- master$Habitat
names(habitat) <- master$KBGenus_Sp

spp <- full_data$species
habitat_df <- as.data.frame(matrix(nrow = length(spp), ncol = 4))
colnames(habitat_df) <- c("species", "Domiciliary", "Peridomiciliary", "Sylvatic")
habitat_df$species <- spp
for (i in 1:length(spp)) {
  habitat_i <- as.character(unique(habitat[names(habitat) == spp[i]]))
  
  list_habitat <- unique(do.call(c, lapply(habitat_i, 
                                           function(x) strsplit(x, "_")[[1]])))
  
  if ("Domiciliary" %in% list_habitat) {
    habitat_df$Domiciliary[habitat_df$species == spp[i]] <- 1
  } else {
    habitat_df$Domiciliary[habitat_df$species == spp[i]] <- 0
  }
  
  if ("Peridomiciliary" %in% list_habitat) {
    habitat_df$Peridomiciliary[habitat_df$species == spp[i]] <- 1
  } else {
    habitat_df$Peridomiciliary[habitat_df$species == spp[i]] <- 0
  }
  
  if ("Sylvatic" %in% list_habitat) {
    habitat_df$Sylvatic[habitat_df$species == spp[i]] <- 1
  } else {
    habitat_df$Sylvatic[habitat_df$species == spp[i]] <- 0
  }
  
}

full_data2 <- full_data %>% 
  inner_join(., habitat_df, by = "species") 

write.csv(full_data2, "data/dietbreadth_niche_habitat_11apr24.csv", 
          row.names = F)
